...
import {
    Leaf, TrendingUp, DollarSign, Recycle,
    ChevronLeft, Cpu, BarChart3, CircleDot,
} from 'lucide-react';
...
const MAT_COLORS: Record<string, string> = {
    ...
    battery:  '#F87171', metal:   '#94A3B8', ewaste:  '#E879F9',
    default:  '#6B7280',
};

function matColor(m: string) { return MAT_COLORS[m] ?? MAT_COLORS.default; }
...
function DonutChart({ data }: { data: { label: string; value: number; color: string }[] }) {
    ...
    return (
        <svg viewBox="0 0 140 140" className="w-36 h-36">
            ...
            <text x={CX} y={CY - 6} textAnchor="middle" className="text-xs" fill="white" fontSize="18" fontWeight="bold">
                {total.toFixed(0)}
            </text>
            ...
            <div className="px-5 lg:px-10 flex flex-col gap-5 max-w-4xl">
                ...
                {donutData.length > 0 && (
                    <div className="card p-5">
                        <p className="text-sm font-bold text-white mb-4" style={{ fontFamily: 'Space Grotesk' }}>
                            Materials Recovered
                        </p>
                                        ...
                                        <div key={m.material} className="flex items-center gap-2">
                                            ...
                                            <div className="flex-1 h-1.5 rounded-full" style={{ background: 'var(--border)' }}>
                                                <div className="h-full rounded-full transition-all duration-500"
                                                    style={{ width: `${pct}%`, background: matColor(m.material) }} />
                                            </div>
                                            <span className="text-[10px] w-12 text-right shrink-0" style={{ color: 'var(--text-secondary)' }}>
                                                {m.massKg} kg
                                            </span>
                                        </div>
                                    );
                ...
                {trend14.length > 1 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="card p-4">
                            <p className="text-xs font-semibold text-white mb-3">CO₂ Avoided — Last 14 Days</p>
                        ...
                        <div className="card p-4">
                            <p className="text-xs font-semibold text-white mb-3">Waste Diverted — Last 14 Days</p>
                        ...
                        <div className="h-4 rounded-full overflow-hidden" style={{ background: 'var(--border)' }}>
                            <div
                                ...
                                style={{
                                    ...
                                    background: s.segregationPct >= 85
